...
*   **Lead Engagement Tracking:** A `lead_engagements` table records all outbound communication attempts (email, WhatsApp).
    ...
    *   Confirmation dispatch is **preference-aware**: when a lead's `preferredContactMethod` is `whatsapp` AND a whatsapp number is on file, the confirmation message (reference number + initial assessment category) is sent via WhatsApp instead of email. Otherwise the email path is used. If there's no email but a whatsapp number exists, whatsapp is used regardless of preference (better to reach the lead via the only contact we have than to silently skip). Both channels share the same body via `composeConfirmationBody()` in `lib/email.ts`.
