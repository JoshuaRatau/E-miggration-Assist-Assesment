...
*   **Admin Lead Management CRM:** A lightweight CRM for operations.
    ...
    *   **Lead Dashboard at `/admin`** (`pages/admin.tsx`): single table with Name (+ ref subline) | Visa Type | WhatsApp (✔/✖) | Status (inline dropdown) | Priority (color-coded inline dropdown — high=red, medium=orange, low=grey) | Next Step (outline badge) | Created | Actions (Contact + View + Send update). Three filters (Status, Priority, WhatsApp) plus a Sort toggle (newest / priority-first). Empty state differentiates "No leads yet" vs "No leads match the current filters." The list endpoint serializes via `serializeLeadAdminList`, which intentionally omits rules-engine fields (`leadScore`, `internalClassification`, `leadCategory`, `adminNotes`) per the "do not expose rules engine data" rule.
    *   **Lead Action Layer (Conversion Engine V1):** Top summary cards show **Total Leads / New / Contacted / Qualified** (sourced from `/stats/summary` `byStatus`). Each lead row carries a derived `nextStep` field — pure function of `leadStatus` via `deriveNextStep()` in `lib/classification.ts` (new→"Review lead", reviewing→"Contact lead", contacted→"Await response", qualified→"Prepare case conversion", converted→"Move to case system", closed/unknown→null). The serializer for every lead-returning admin endpoint (`GET /api/leads`, `GET /api/leads/by-id/:id`, `PATCH /api/admin/leads/:id`) includes `nextStep` so payloads stay consistent. The frontend mirrors the same map locally so the column updates instantly on optimistic status edits without waiting for refetch. A subtle row tint (`bg-blue-50/60`) flags `new`/`reviewing` leads as "needs your attention". A per-row **Contact** button deep-links to WhatsApp (`https://wa.me/<digits-only>`, opens in new tab) when the lead has a WhatsApp number, falls back to `mailto:`, and is disabled with an explanatory tooltip when neither is available.
