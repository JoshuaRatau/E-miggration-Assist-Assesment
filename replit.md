...
**Chrome v3.1 (Phase 5H — topbar personalisation + brand palette):** Three small but visible polish moves on top of v3. (1) Removed the "Dashboard"/page-title pill that sat next to the logo; replaced with `<TopbarGreeting/>` which renders the personalised "Good afternoon, Demo" line (gated by `useAdminAuth().user`, hidden below sm). (2) Added `<TopbarClock/>` to the right side of the topbar (left of the Admin button), showing live "Mon, 8 May · 12:38 SAST" — hidden below md. Both components share a new `useMinuteClock()` hook in `dashboard-greeting.tsx` (single minute-aligned setInterval rather than two competing timers). (3) `DashboardGreeting` slimmed to just the title block ("Lead Intelligence Dashboard" h1 + subtitle); the greeting/clock pair was promoted into the global topbar so it surfaces on every admin page, not just the dashboard. (4) `BRAND_GRADIENT` swapped from the v3 radial (`#173768 → #0c1d3a → #0a162b`) to a diagonal linear gradient aligned to the marketing site's hero image — `linear-gradient(135deg, #11618c 0%, #114c7e 30%, #0f3667 60%, #0b234a 100%)` — visibly lighter and less saturated; cards still elevate on top via `bg-card`. (5) `AdminLayout`'s `title` prop is now accepted but unused (back-compat only — the 9 admin pages that still pass it don't need a code change). The greeting strip's text is intentionally white/`text-white/70` because the topbar background is `bg-slate-950/60`.

**Chrome v3 (Phase 5G — sidebarless executive shell):** Reverses the Phase 5F sidebar+launcher hybrid in favour of a single top-right Admin dropdown. Goal was to remove visual compression and lift the dashboard to a premium SaaS aesthetic (HubSpot/Linear/Notion reference set). (1) `components/admin-layout.tsx` rewritten — left sidebar removed entirely (both desktop rail and mobile sheet drawer); main column now spans full width up to `max-w-[1600px]`; topbar height bumped to `h-16 sm:h-20` to accommodate an enlarged brand logo (`h-12 sm:h-16`) wrapped in a `Link href="/admin"` so clicking the logo from any admin page returns to Dashboard. (2) `components/admin-user-menu.tsx` rebuilt as the SOLE navigation surface — trigger label is now the generic "Admin" (was the user's display name), four grouped sections (Workspace / Operations / Intelligence / Admin) covering all 12 modules, explicit Logout row at the bottom. Identity (display name + email + superadmin badge) is preserved inside the menu header so operators still see which account is active. (3) `components/workspace-launcher.tsx` deleted — its responsibility folded into the unified Admin dropdown. (4) Body palette switched to a radial gradient anchored on the `#3381C7` brand accent (`radial-gradient(ellipse at top, #173768 0%, #0c1d3a 55%, #0a162b 100%)`) — darker elevated cards (`bg-card`) sit on top, preserving readability. The literal flat `#3381C7` background was rejected as visually heavy; cards/buttons keep the brand blue as accent only. (5) `admin.tsx` lost its inline Export/Import button row (and the now-orphan `handleExportCsv` + `exportingCsv` state) — bulk CSV export lives at `/admin/exports`, lead imports at `/admin/import`, both reachable from the Admin dropdown. Per-row CSV/timeline/send-update actions on the leads table are untouched. (6) `pages/admin.tsx` `bodyClassName` override removed so the dashboard inherits the new brand gradient; `space-y-10 → space-y-12` and `pt-2 → pt-4` give the heading more vertical breathing room. (7) Topbar styling — `bg-slate-950/60 backdrop-blur` with white-on-dark text/border tokens; the page-title pill renders muted (`text-white/70`) with a left-border separator so it reads as breadcrumb context, not page heading. **Other admin pages keep their per-page bodyClassName overrides** (campaign editor/detail still on `bg-slate-950`, profile/users use the default gradient).

**Chrome v2 (Phase 5F — refinement pass):** Replaced the action-heavy dashboard topbar with a workspace launcher dropdown and lightened the visual density. (1) `components/workspace-launcher.tsx` — top-right Radix `DropdownMenu` exposing the FULL 12-module list (Dashboard, Leads, Pipelines, Communications, Campaigns, Imports, Exports, Analytics, Reports, Subscriptions, Customer Support, Manage Admins [super-only], Settings). The launcher is the single source of truth for "what modules exist"; the persistent left sidebar carries only the daily-driver subset (Dashboard / Communications / Imports / Manage Admins / Settings) so navigation cost stays at one click for hot paths. Both surfaces coexist deliberately — pure-dropdown nav was rejected as a regression on the executive-CRM reference set (Linear/Notion/HubSpot/Zoho all keep a left rail). (2) Removed three topbar buttons from `admin.tsx`: "Send Update Email" (retired entirely; the per-row Send Update dialog still exists for targeted sends), "Export Leads (CSV)" (relocated — see below), "Import Professionals" (renamed to "Import Leads" and demoted to a small inline body button next to the export quick-action). All `handleSendUpdateEmail`-related state/url/handler dead code was deleted. (3) New routes wired in `App.tsx`, all wrapped in `RequireAdminAuth`: `/admin/analytics`, `/admin/reports`, `/admin/subscriptions`, `/admin/support`, `/admin/pipelines` (all stub pages in `pages/admin-stub.tsx`) + `/admin/exports` (real page in `pages/admin-exports.tsx` that exposes the CSV export). Stubs render inside `AdminLayout` with a clean placeholder card so launcher items never dead-click. Settings is a label rebrand of `/admin/profile` — same route, same page. (4) `AdminExports` and the inline body button on `admin.tsx` both call the existing `GET /api/leads/export.csv` endpoint with `credentials: "include"` (cookie auth). (5) `DashboardGreeting` rebalanced for breathing room — wrapped in `space-y-6` with explicit `space-y-2` between greeting/clock and `space-y-3` between heading/subtitle; heading bumped from `text-2xl/3xl` to `text-3xl/4xl` with `tracking-tight`. (6) Dashboard `bodyClassName` overridden with `bg-gradient-to-b from-slate-800 to-slate-900` — softens the previous near-black without losing the dark-theme aesthetic. Other admin pages keep their own `bodyClassName` (campaign editor/detail stay on `bg-slate-950`).
