...
Replaces the dashboard's simple source-mix table with a CRM-style attribution panel. **Backend:** new `GET /api/stats/source-attribution?range=7d|30d|1m|3m|6m|all&segment=all|b2c|b2b` route in `routes/stats.ts` (admin-gated, deliberately bespoke — NOT in OpenAPI to avoid codegen churn while shape iterates). Returns `{rows, totals, series, insights, range, segment, bucket}`: per-source rows include `leads`, `converted`, `conversionPct`, `growthPct` (period-over-period; `null` for new channels with prev=0+leads>0, also `null` for `range=all`); `series` is daily/weekly/monthly buckets pivoted by source for the chart (bucket width auto-selected: 7d/30d/1m→day, 3m/6m→week, all→month); `insights` is server-derived (top volume, top conv-rate ≥3 leads, fastest-grower >25%, decliner <-25%). Inputs are validated against closed allow-lists before any `sql.raw` interpolation, so dynamic SQL is bounded. **Optimization:** `range=all` short-circuits the correlated `prev_leads` subquery to `0::int` since growth is null anyway. **Legacy `/stats/source-mix` route preserved** — nothing else points at it, but keeping it cheap-and-stable in case any operator script calls it. **Frontend:** `components/source-performance-card.tsx` rebuilt — time-range pills, segment toggle (All/Travellers/Firms), Table↔Graph view toggle, totals strip (leads/converted/overall conv%), recharts `ComposedChart` with smooth `Line`s + gradient `Area` fills (top 6 sources foregrounded with deterministic colour assignment, remainder rendered as faint background lines for context), insights panel with tone-coloured icons (positive/neutral/warning), Growth% badge column. Uses `useQuery` directly with 30s `staleTime` (no orval hook). **V2 deferred:** zero-fill missing buckets server-side (currently empty days are omitted, can visually compress quiet periods); avg qualification score / avg response time / pipeline progression columns (not granularly tracked yet).
