...
*   **Inbound WhatsApp Webhook (Phase 1 — deterministic):** Inbound replies from leads are received via Meta's webhook delivery and stored for operator review.
    *   `GET /api/webhooks/whatsapp` — Meta verification handshake. Compares `hub.verify_token` against `WHATSAPP_VERIFY_TOKEN` in constant time and echoes `hub.challenge` on match.
    *   `POST /api/webhooks/whatsapp` — receives Meta event payloads. ALWAYS responds 200 (Meta retries non-2xx for 24h). HMAC-SHA256 signature verification against `X-Hub-Signature-256` using `WHATSAPP_APP_SECRET` over the RAW request body — `app.ts` captures the raw body via `express.json({ verify })` so JSON re-serialization can't corrupt the comparison.
    *   Inbound messages are stored in the new `case_messages` table (`lead_id`, `direction='inbound'`, `wa_message_id` UNIQUE for idempotency, `message`, `intent`, `matched_keyword`). Lead matching is by canonical WhatsApp number via `normalizeWhatsapp`; unknown numbers are dropped with an info log.
    ...
    *   Required secrets: `WHATSAPP_APP_SECRET` (Meta App Dashboard → Settings → Basic), `WHATSAPP_VERIFY_TOKEN` (any string you choose; must also be entered in the Meta webhook config).
